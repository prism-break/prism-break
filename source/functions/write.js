...
(function(){
  'use strict';
  var mkdirp, helpers, sortBy, ref$, selectRandom, slugifyDb, slugifyProject, subcategoriesOf, imagesIn, inThisCategory, inThisSubcategory, inTheseSubcategories, inThisProtocol, nestedCategories, nestedCategoriesWeb, platformTypes, protocolTypes, writeHtml, writeJson, routes, viewPath, writeLocalizedSite, writeSiteIndex, writeCategoriesIndex, writeCategoriesShow, writeSubcategoriesShow, writeProtocolsIndex, writeProtocolsShow, writeProjectsIndex, writeProjectsShow, writeAboutIndex, writeAboutMedia;
  ...
  writeCategoriesShow = function(db){
    ...
    create = function(category){
      var data, i$, ref$, len$, subcategory, rejected, path, view, options, fullPath, file, write;
      data = category;
      for (i$ = 0, len$ = (ref$ = data.subcategories).length; i$ < len$; ++i$) {
        subcategory = ref$[i$];
        rejected = inThisSubcategory(subcategory.name, inThisCategory(category.name, db.projectsRejected));
        subcategory.projectsRejected = sortBy(fn$, rejected);
      }
